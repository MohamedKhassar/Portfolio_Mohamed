...
const page = ({ params: { name } }: { params: { name: string } }) => {
    ...
    return (
        <div className="h-[90vh] flex flex-col justify-center items-center gap-5">
            <h1 className='text-wrap capitalize font-bold font-Poppins text-3xl lg:text-5xl flex items-center mx-3'>{project?.title}<span className='lg:text-6xl text-4xl font-bold text-[#6e06f2]'>.</span></h1>
            <div className="rounded-3xl lg:w-[30rem] w-72  p-5 shadow-2xl lg:flex flex-col gap-5 hover:shadow-[#6e06f2]/70 duration-300 hover:scale-105 lg:space-y-0 space-y-10" key={project?.id}>
                <Image className="rounded-md lg:h-60 object-cover w-full h-[8.5rem]" src={project!.image} width={1000} height={1000} alt={project!.image} />
